...
def test_prog_intermed_sign_invalid_dn(mocker, tmp_path):
    ...
    # Aufruf der Funktion
    # from ftwpki.intermed_signer.programms import prog_intermed_sign
...
def test_prog_intermed_sign_path_length_error(mocker):
    ...
    mocker.patch("ftwpki.intermed_signer.programms.load_certificate_from_pem", return_value=mock_cert)
...
def test_prog_intermediate_sign_general_exception(mocker):
    ...
    mocker.patch(
        "ftwpki.intermed_signer.programms.load_private_key_from_pem", side_effect=RuntimeError("Krawumm")
    )
...
def test_prog_intermediate_sign_interrupt(mocker):
    mocker.patch(
        "ftwpki.intermed_signer.programms.CSRMultiSigningParser.parse_args", side_effect=KeyboardInterrupt
    )
    assert prog_intermediate_sign([]) == 1

def test_prog_intermediate_sign_exception(mocker):
    mocker.patch(
        "ftwpki.intermed_signer.programms.CSRMultiSigningParser.parse_args",
...
def test_prog_intermediate_sign_full_flow(mocker):
    ...
    mocker.patch("ftwpki.intermed_signer.programms.load_private_key_from_pem")
    ...
    mocker.patch("ftwpki.intermed_signer.programms.PasswordManager")

    # 5. Mocke den Signier-Vorgang und Export
    mocker.patch("ftwpki.intermed_signer.programms.CertificateSigner")
    mocker.patch("ftwpki.intermed_signer.programms.validate_and_clamp_validity")
    mocker.patch(
        "ftwpki.intermed_signer.programms.encrypt_transport_package", return_value=b"zipped_content"
    )
...
def test_prog_intermediate_sign_validation_fails(mocker):
    ...
    mock_val_class = mocker.patch("ftwpki.intermed_signer.programms.ValidatorDN")
